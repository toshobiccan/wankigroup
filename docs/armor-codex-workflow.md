...
2. Read project.json and prompt.txt. View mannequin-template.png and the style references `assets/art-reference/human-base.png` and `assets/art-reference/goblin-mace-medic.png` before generation.
3. Use the built-in image-generation tool with those three images, the complete prompt, and a transparent background. Do not substitute an API or generate an assembled character. Keep distinct front/rear pieces and exact sheet cell order.
