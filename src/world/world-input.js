...
export class WorldInput {
  constructor(mount, onAction) {
    ...
    const update=e=>{const r=pad.getBoundingClientRect(),x=(e.clientX-r.left-r.width/2)/(r.width/2),y=(e.clientY-r.top-r.height/2)/(r.height/2);this.pad=movementVector(Math.abs(x)>.23?Math.sign(x):0,Math.abs(y)>.23?Math.sign(y):0);};
    pad.addEventListener('pointerdown',e=>{if(!this.active || pointer!==null)return;e.preventDefault();pointer=e.pointerId;pad.setPointerCapture(pointer);update(e);});
    ...
    this.root.querySelectorAll('button').forEach((b,i)=>b.addEventListener('click',()=>{if(this.active)onAction(i?'confirm':'cancel');}));
    window.addEventListener('keydown',e=>{
      ...
      if(KEYS[e.code]){e.preventDefault();this.keys.add(e.code);}
      else if(!e.repeat && ['KeyE','Space','Escape'].includes(e.code)){e.preventDefault();onAction(e.code==='Escape'?'cancel':'confirm');}
    },{signal});
  ...
  get vector(){if(modalOpen()){this.reset();return {x:0,y:0};}let x=this.pad.x,y=this.pad.y;for(const key of this.keys){x+=KEYS[key][0];y+=KEYS[key][1];}return this.active?movementVector(x,y):{x:0,y:0};}
