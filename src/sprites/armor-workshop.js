...
export const STYLE_REFERENCES = ["human-base-v5-front-grip.png"];
...
human-base-v5-front-grip.png controls line weight, palette discipline, cel shading and readable detail. Do not copy its pose, shorts, boots or background into an armor sheet; the mannequin remains the geometry guide.
...
Use a restrained shared material palette across the outfit, with a base plus TWO darker shadow tones per material. Include readable angular folds, seams, plate overlaps and a few large fasteners. No shiny rendering, tiny stitching or repeated decorative buckles. Keep the thick outer contour visually dominant, including at thumbnail scale. Hand-drawn means clean but slightly irregular angular contours, not sketch hatching or surface noise.

CUTOUT SHEET REQUIREMENTS (these replace full-character composition and pose instructions):
${project.slot === "armor" ? `ARMOR DETAIL — match the reference's medium detail, bold ink and angular handmade shapes. Keep readable structural details rather than reducing every part to a blank color block.
Use at most THREE main material colors across the complete outfit, plus near-black outlines. Each material has one flat base color and TWO darker cel-shadow colors. Use a few coherent angular shadow planes and restrained edge details. No broad gradients, mottling or glossy highlights. Rear pieces use the same shadow palette, not extra shades.
...
Keep a clear silhouette per limb section. Allow a few purposeful seams, cloth folds, armor overlaps and large fasteners that explain construction. Avoid tiny rivet rows and straps wrapped around every limb. Boots and gloves should share the outfit materials and two shadow tones. Remove marks that disappear at 150 pixels character height. Aim for an economical hand-drawn Flash sprite, not a miniature detailed armor illustration.
